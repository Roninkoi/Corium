...
//TextRenderer textRenderer;
	...
	void drawScreen(Renderer *renderer)
	{
		...
		guiRenderer.update(renderer);

		gui.drawGUI(&guiRenderer);
	...
	void drawLoadingScreen(Renderer *renderer)
	{
		...
		crmlogo.phys.update();
		crmlogo.update();
		guiRenderer.draw(crmlogo.tex, glm::mat4(1.0f), &crmlogo.vertexData, &crmlogo.texData,
	...
	void drawText(std::string s, Quad textQuad)
	{
		for (int i = 0; i < s.length(); ++i) {
			...
			textQuad.update();
			guiRenderer.draw(textQuad.tex, glm::mat4(1.0f), &textQuad.vertexData, &textQuad.texData,
